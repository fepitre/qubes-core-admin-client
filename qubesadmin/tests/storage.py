...
class TestVMVolume(qubesadmin.tests.QubesTestCase):
    def setUp(self):
        ...
        self.vol = qubesadmin.storage.Volume(
            self.app, vm="test-vm", vm_name="volname"
        )
        self.pool_vol = qubesadmin.storage.Volume(
            self.app, pool="test-pool", vid="some-id"
        )
    ...
    def test_000_qubesd_call(self):
        ...
        self.assertEqual(self.vol._qubesd_call("TestMethod"), b"method_result")
    ...
    def test_021_revisions(self):
        ...
        self.assertEqual(self.vol.revisions, ["snapid1", "snapid2", "snapid3"])
        self.assertAllCalled()
    ...
    def test_040_import_data(self):
        ...
        input_proc = subprocess.Popen(
            ["echo", "-n", "some-data"], stdout=subprocess.PIPE
        )
...
class TestPoolVolume(TestVMVolume):
    ...
    def test_000_qubesd_call(self):
        self.app.expected_calls[
            ("dom0", "admin.pool.volume.TestMethod", "test-pool", b"some-id")
        ] = b"0\x00method_result"
        self.assertEqual(self.vol._qubesd_call("TestMethod"), b"method_result")
        self.assertAllCalled()
    ...
    def test_021_revisions(self):
        self.app.expected_calls[
            ("dom0", "admin.pool.volume.ListSnapshots", "test-pool", b"some-id")
        ] = (b"0\x00" b"snapid1\n" b"snapid2\n" b"snapid3\n")
        self.assertEqual(self.vol.revisions, ["snapid1", "snapid2", "snapid3"])
        self.assertAllCalled()
    ...
    def test_030_resize(self):
        self.app.expected_calls[
            ("dom0", "admin.pool.volume.Resize", "test-pool", b"some-id 2048")
        ] = b"0\x00"
        self.vol.resize(2048)
        self.assertAllCalled()
    ...
    def test_050_clone(self):
        self.app.expected_calls[
            ("dom0", "admin.pool.volume.CloneFrom", "test-pool", b"volid")
        ] = b"0\x00abcdef"
        self.app.expected_calls[
            (
                "dom0",
        ...
        source_vol = qubesadmin.storage.Volume(
            self.app, pool="test-pool", vid="volid"
        )
...
class TestPool(qubesadmin.tests.QubesTestCase):
    ...
    def test_030_pool_drivers(self):
        ...
        self.assertEqual(
            set(self.app.pool_driver_parameters("file")),
    ...
    def test_040_add(self):
        ...
        self.app.add_pool(
            "test-pool", driver="some-driver", param1="value1", param2=123
        )
        self.assertAllCalled()

    def test_050_remove(self):
        self.app.expected_calls[
            ("dom0", "admin.pool.Remove", "test-pool", None)
        ] = b"0\x00"
