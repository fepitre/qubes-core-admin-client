...
parsed_qubes_xml_r2 = {
    "domains": {
        ...
        "fedora-20-x64-dvm": {
            "klass": "AppVM",
            "label": "gray",
            "properties": {"maxmem": "1535", "vcpus": "1"},
            "devices": {},
            "tags": set(),
            "features": {"internal": True, "service.meminfo-writer": True},
            "template": "fedora-20-x64",
            "backup_path": None,
            "included_in_backup": False,
        },
        ...
        "disp-no-netvm": {
            "klass": "AppVM",
            "label": "red",
            "properties": {"netvm": None, "template_for_dispvms": True},
            "devices": {},
            "features": {},
            "tags": set(),
...
parsed_qubes_xml_v4 = {
    "domains": {
        ...
        "fedora-25-dvm": {
            "klass": "AppVM",
            "label": "red",
            "properties": {"template_for_dispvms": "True", "vcpus": "1"},
            "devices": {},
            "tags": set(),
            "features": {"internal": "1", "service.meminfo-writer": "1"},
            "template": "fedora-25",
            "backup_path": None,
            "included_in_backup": False,
        },
        "fedora-25-clone-dvm": {
            "klass": "AppVM",
            "label": "red",
            "properties": {"vcpus": "1", "template_for_dispvms": "True"},
            "devices": {},
            "tags": set(),
            "features": {"internal": "1", "service.meminfo-writer": "1"},
...
class TC_00_QubesXML(qubesadmin.tests.QubesTestCase):
    def assertCorrectlyConverted(self, backup_app, expected_data):
        self.assertCountEqual(
            backup_app.domains.keys(), expected_data["domains"].keys()
        )
        for vm in expected_data["domains"]:
            self.assertEqual(backup_app.domains[vm].name, vm)
            self.assertEqual(
                backup_app.domains[vm].properties,
                expected_data["domains"][vm]["properties"],
                vm,
            )
            for devtype in expected_data["domains"][vm]["devices"]:
                self.assertEqual(
                    backup_app.domains[vm].devices[devtype],
                    expected_data["domains"][vm]["devices"][devtype],
                    vm,
                )
            self.assertEqual(
                backup_app.domains[vm].tags,
                expected_data["domains"][vm]["tags"],
                vm,
            )
            self.assertEqual(
                backup_app.domains[vm].features,
                expected_data["domains"][vm]["features"],
                vm,
            )
            self.assertEqual(
                backup_app.domains[vm].template,
                expected_data["domains"][vm]["template"],
                vm,
            )
            self.assertEqual(
                backup_app.domains[vm].klass,
                expected_data["domains"][vm]["klass"],
                vm,
            )
            self.assertEqual(
                backup_app.domains[vm].label,
                expected_data["domains"][vm]["label"],
                vm,
            )
            self.assertEqual(
                backup_app.domains[vm].backup_path,
                expected_data["domains"][vm]["backup_path"],
                vm,
            )
            self.assertEqual(
                backup_app.domains[vm].included_in_backup,
                expected_data["domains"][vm]["included_in_backup"],
                vm,
            )
...
class AppProxy(object):
    ...
    def qubesd_call(
        ...
        if payload_stream:
            ...
            subprocess.call(
                ["cat"], stdin=payload_stream, stdout=subprocess.DEVNULL
            )
...
@unittest.skipUnless(
    os.environ.get("ENABLE_SLOW_TESTS", False),
...
class TC_10_BackupCompatibility(qubesadmin.tests.backup.BackupTestCase):
    ...
    def create_v1_files(self, r2b2=False):
        ...
        os.symlink(
            "/usr/share/qubes/icons/blue.png",
        ...
        if r2b2:
            ...
            self.fill_image(
                self.fullpath("appvms/test-testhvm/root.img"),
                1024 * 1024,
                True,
        ...
        self.create_whitelisted_appmenus(
            self.fullpath("servicevms/test-net/whitelisted-appmenus.list")
        )
        self.create_private_img(
    ...
    def create_v4_files(self):
        ...
        for vm in (
            ...
            self.create_whitelisted_appmenus(
                self.fullpath("appvms/{}/whitelisted-appmenus.list".format(vm))
            )
            self.create_private_img(
                self.fullpath("appvms/{}/private.img".format(vm))
            )
        ...
        for vm in ("test-standalonevm", "test-hvm"):
            ...
            self.fill_image(
                self.fullpath("appvms/{}/root.img".format(vm)),
                1024 * 1024,
                True,
        ...
        self.create_sparse(
            self.fullpath("vm-templates/test-fedora-25-clone/root.img"),
            10 * 2 ** 20,
        )
        self.fill_image(
            self.fullpath("vm-templates/test-fedora-25-clone/root.img"),
        ...
        self.create_volatile_img(
            self.fullpath("vm-templates/test-fedora-25-clone/volatile.img")
        )
        self.create_whitelisted_appmenus(
            self.fullpath(
        ...
        self.create_appmenus(
            ...
            APPTEMPLATE_R2B2.replace(
                "%VMNAME%", "test-fedora-25-clone"
            ).replace(
                "%VMDIR%", self.fullpath("vm-templates/test-fedora-25-clone")
            ),
            appmenus_list,
        )
    ...
    def scrypt_encrypt(
        self, f_name, output_name=None, password="qubes", basedir=None
    ):
        if basedir is None:
    ...
    def handle_v4_file(self, f_name, subdir, stream, compressed="gzip"):
        ...
        for part in sorted(os.listdir(stage1_dir)):
            if not re.match(
                ...
                continue
            part_with_dir = os.path.join(subdir, part)
            f_name = self.scrypt_encrypt(
                part_with_dir, basedir=self.fullpath("stage1")
            )
            self.append_backup_stream(
                f_name, stream, basedir=self.fullpath("stage1")
            )
    ...
    def create_v4_backup(self, compressed="gzip"):
        ...
        self.handle_v4_file(
            "dom0-home/user", "dom0-home/", output, compressed=compressed
        )
    ...
    def setup_expected_calls(self, parsed_qubes_xml, templates_map=None):
        ...
        for name, vm in parsed_qubes_xml["domains"].items():
            ...
            for prop, value in vm["properties"].items():
                self.app.expected_calls[
                    (
                        ...
                        prop,
                        str(value).encode() if value is not None else b"",
                    )
                ] = b"0\0"
            ...
            for bus, devices in vm["devices"].items():
                for (backend_domain, ident), options in devices.items():
                    ...
                    self.app.expected_calls[
                        (
                            name,
                            "admin.vm.device.{}.Attach".format(bus),
            ...
            for feature, value in vm["features"].items():
                if value is False:
                    ...
                    value = "1"
                self.app.expected_calls[
                    (name, "admin.vm.feature.Set", feature, str(value).encode())
                ] = b"0\0"

            for tag in vm["tags"]:
                self.app.expected_calls[
                    (name, "admin.vm.tag.Set", tag, None)
                ] = b"0\0"
        ...
        self.app.expected_calls[("dom0", "admin.vm.List", None, None)] = [
            orig_admin_vm_list
        ] + [orig_admin_vm_list + b"".join(extra_vm_list_lines)] * len(
            extra_vm_list_lines
        )
    ...
    def test_210_r2(self):
        ...
        self.setup_expected_calls(
            parsed_qubes_xml_r2, templates_map={"fedora-20-x64": "fedora-25"}
        )
        firewall_data = (
        ...
        self.app.expected_calls[
            ("test-work", "admin.vm.firewall.Set", None, firewall_data.encode())
        ] = b"0\0"
        self.app.expected_calls[
            (
                "test-custom-template-appvm",
    ...
    @unittest.skipUnless(
        spawn.find_executable("scrypt"), "scrypt not installed"
    )
    def test_230_r4(self):
        ...
        self.setup_expected_calls(
            parsed_qubes_xml_v4, templates_map={"debian-8": "fedora-25"}
        )
        firewall_data = (
            "action=accept specialtarget=dns\n"
            "action=accept proto=icmp\n"
...
class TC_11_BackupCompatibilityIntoLVM(TC_10_BackupCompatibility):
    ...
    def restore_backup(self, source=None, appvm=None, options=None, **kwargs):
        ...
        super(TC_11_BackupCompatibilityIntoLVM, self).restore_backup(
