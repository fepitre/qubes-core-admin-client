...
class TC_00_RuleAction(qubesadmin.tests.QubesTestCase):
    ...
    def test_003_mixed_opts(self):
        ns = argparse.Namespace()
        self.action(
            None, ns, ["dsthost=127.0.0.1", "accept", "dstports=443", "tcp"]
        )
        self.assertEqual(
            ns.rule,
            qubesadmin.firewall.Rule(
    ...
    def test_004_expire_absolute(self):
        ...
        self.action(
            None,
            ns,
            ["dsthost=127.0.0.1", "action=accept", "expire=1525054180"],
        )
        self.assertEqual(
            ns.rule,
            qubesadmin.firewall.Rule(
                None, action="accept", dsthost="127.0.0.1/32", expire=1525054180
            ),
        )
    ...
    def test_005_expire_relative(self):
        ...
        self.action(
            None, ns, ["dsthost=127.0.0.1", "action=accept", "expire=+100"]
        )
        self.assertEqual(
            ns.rule,
            qubesadmin.firewall.Rule(
                None, action="accept", dsthost="127.0.0.1/32", expire=now + 100
            ),
        )
...
class TC_10_qvm_firewall(qubesadmin.tests.QubesTestCase):
    ...
    def test_003_list_raw_reload(self):
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.firewall.Reload", None, None)
        ] = b"0\0"
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            qubesadmin.tools.qvm_firewall.main(
                ["test-vm", "--raw", "--reload", "list"], app=self.app
            )
            self.assertEqual(
                [l.strip() for l in stdout.getvalue().splitlines()],
                [
    ...
    def test_020_del_number(self):
        ...
        qubesadmin.tools.qvm_firewall.main(
            ["test-vm", "del", "--rule-no", "1"], app=self.app
        )
    ...
    def test_021_del_rule(self):
        ...
        qubesadmin.tools.qvm_firewall.main(
            ["test-vm", "del", "drop", "proto=icmp"], app=self.app
        )
