...
class TC_00_qvm_template_postprocess(qubesadmin.tests.QubesTestCase):
    ...
    def test_000_import_root_img_raw(self):
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.volume.Import", "root", volume_data)
        ] = b"0\0"
    ...
    def test_001_import_root_img_tar(self):
        ...
        subprocess.check_call(
            ["tar", "cf", "root.img.tar", "root.img"], cwd=self.source_dir.name
        )
        subprocess.check_call(
            ["split", "-d", "-b", "1024", "root.img.tar", "root.img.part."],
    ...
    def test_002_import_root_img_no_overwrite(self):
        ...
        template_dir = os.path.join(
            self.source_dir.name, "vm-templates", "test-vm"
        )
    ...
    def test_005_reset_private_img(self):
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.volume.Import", "private", b"")
        ] = b"0\0"
    ...
    def test_010_import_appmenus(self):
        with open(
            os.path.join(self.source_dir.name, "vm-whitelisted-appmenus.list"),
            "w",
        ) as f:
            f.write("org.gnome.Terminal.desktop\n")
            f.write("firefox.desktop\n")
        with open(
            os.path.join(self.source_dir.name, "whitelisted-appmenus.list"), "w"
        ) as f:
            f.write("org.gnome.Terminal.desktop\n")
            ...
            [
                mock.call(
                    [
                        ...
                        + os.path.join(
                            self.source_dir.name, "vm-whitelisted-appmenus.list"
                        ),
                        "test-vm",
                    ]
                ),
                mock.call(
                    [
                        ...
                        "--set-whitelist="
                        + os.path.join(
                            self.source_dir.name, "whitelisted-appmenus.list"
                        ),
                        "test-vm",
                    ]
                ),
    ...
    def test_020_post_install(self, mock_import_root_img, mock_import_appmenus):
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.property.Set", "netvm", b"")
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm", "admin.vm.property.Set", "installed_by_rpm", b"True")
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm", "admin.vm.property.Reset", "netvm", None)
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Set", "qrexec", b"1")
        ] = b"0\0"
        self.app.expected_calls[
        ...
        self.app.add_new_vm.assert_called_once_with(
            "TemplateVM", name="test-vm", label="black"
        )
        mock_import_root_img.assert_called_once_with(
            self.app.domains["test-vm"], self.source_dir.name
        )
        mock_import_appmenus.assert_called_once_with(
            self.app.domains["test-vm"], self.source_dir.name
        )
        if qubesadmin.tools.qvm_template_postprocess.have_events:
            mock_domain_shutdown.assert_called_once_with(
    ...
    def test_021_post_install_reinstall(
        self, mock_reset_private_img, mock_import_root_img, mock_import_appmenus
    ):
        self.app.expected_calls[
            ("dom0", "admin.vm.List", None, None)
        ] = b"0\0test-vm class=TemplateVM state=Halted\n"
        ...
        mock_reset_private_img.assert_called_once_with(
            self.app.domains["test-vm"]
        )
        mock_import_appmenus.assert_called_once_with(
            self.app.domains["test-vm"], self.source_dir.name
    ...
    @mock.patch("qubesadmin.tools.qvm_template_postprocess.import_root_img")
    @mock.patch("qubesadmin.tools.qvm_template_postprocess.reset_private_img")
    def test_022_post_install_skip_start(
        self, mock_reset_private_img, mock_import_root_img, mock_import_appmenus
    ):
        self.app.expected_calls[
            ("dom0", "admin.vm.List", None, None)
    ...
    def test_030_pre_remove(self):
        ...
        ret = qubesadmin.tools.qvm_template_postprocess.main(
            ["--really", "pre-remove", "test-vm", self.source_dir.name],
            app=self.app,
        )
        self.assertEqual(ret, 0)
        self.assertEqual(self.app.service_calls, [])
    ...
    def test_040_missing_really(self):
        with self.assertRaises(SystemExit):
            qubesadmin.tools.qvm_template_postprocess.main(
                ["post-install", "test-vm", self.source_dir.name], app=self.app
            )
        self.assertAllCalled()
