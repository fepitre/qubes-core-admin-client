...
class TC_00_qvm_features(qubesadmin.tests.QubesTestCase):
    def test_000_list(self):
        ...
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            self.assertEqual(
                qubesadmin.tools.qvm_features.main(["some-vm"], app=self.app), 0
            )
            self.assertEqual(
                stdout.getvalue(),
    ...
    def test_001_set(self):
        ...
        self.app.expected_calls[
            ("some-vm", "admin.vm.feature.Set", "feature3", b"value of feature")
        ] = b"0\x00"
        self.assertEqual(
            qubesadmin.tools.qvm_features.main(
                ["some-vm", "feature3", "value of feature"], app=self.app
            ),
            0,
        )
        self.assertAllCalled()
    ...
    def test_002_get(self):
        ...
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            self.assertEqual(
                qubesadmin.tools.qvm_features.main(
                    ["some-vm", "feature3"], app=self.app
                ),
                0,
            )
            self.assertEqual(stdout.getvalue(), "value2 with spaces\n")
        self.assertAllCalled()
    ...
    def test_003_del(self):
        ...
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            self.assertEqual(
                qubesadmin.tools.qvm_features.main(
                    ["--unset", "some-vm", "feature4"], app=self.app
                ),
                0,
            )
