...
class TC_00_qvm_tags(qubesadmin.tests.QubesTestCase):
    def test_000_list(self):
        ...
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            self.assertEqual(
                qubesadmin.tools.qvm_tags.main(["some-vm"], app=self.app), 0
            )
    ...
    def test_002_list_alias(self):
        ...
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            self.assertEqual(
                qubesadmin.tools.qvm_tags.main(["some-vm", "ls"], app=self.app),
                0,
            )
            self.assertEqual(stdout.getvalue(), "tag1\n" "tag2\n")
        self.assertAllCalled()
    ...
    def test_010_add(self):
        ...
        self.app.expected_calls[
            ("some-vm", "admin.vm.tag.Set", "tag3", None)
        ] = b"0\x00"
        self.assertEqual(
            qubesadmin.tools.qvm_tags.main(
                ["some-vm", "add", "tag3"], app=self.app
            ),
            0,
        )
        self.assertAllCalled()
    ...
    def test_020_del(self):
        ...
        self.app.expected_calls[
            ("some-vm", "admin.vm.tag.Remove", "tag3", None)
        ] = b"0\x00"
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            self.assertEqual(
                qubesadmin.tools.qvm_tags.main(
                    ["some-vm", "del", "tag3"], app=self.app
                ),
                0,
            )
            self.assertEqual(stdout.getvalue(), "")
