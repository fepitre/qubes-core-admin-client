...
class TC_00_qvm_create(qubesadmin.tests.QubesTestCase):
    ...
    def test_002_custom_template(self):
        ...
        qubesadmin.tools.qvm_create.main(
            ["-l", "red", "-t", "some-template", "new-vm"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_003_properties(self):
        ...
        self.app.expected_calls[
            ("new-vm", "admin.vm.property.Set", "netvm", b"sys-whonix")
        ] = b"0\x00"
        qubesadmin.tools.qvm_create.main(
            ["-l", "red", "--prop", "netvm=sys-whonix", "new-vm"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_004_pool(self):
        ...
        qubesadmin.tools.qvm_create.main(
            ["-l", "red", "-P", "some-pool", "new-vm"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_009_help_classes(self):
        self.app.expected_calls[
            ("dom0", "admin.vmclass.List", None, None)
        ] = b"0\x00StandaloneVM\nAppVM\nTemplateVM\nDispVM\n"
        with qubesadmin.tests.tools.StdoutBuffer() as stdout:
            qubesadmin.tools.qvm_create.main(["--help-classes"], app=self.app)
            self.assertEqual(
                stdout.getvalue(), "AppVM\nDispVM\nStandaloneVM\nTemplateVM\n"
            )
        self.assertAllCalled()
    ...
    def test_011_standalonevm(self, check_output_mock):
        ...
        qubesadmin.tools.qvm_create.main(
            ["-C", "StandaloneVM", "-t", "template", "-l", "red", "new-vm"],
            app=self.app,
        )
    ...
    def test_014_standalone_shortcut(self):
        ...
        qubesadmin.tools.qvm_create.main(
            ["-l", "red", "--standalone", "new-vm"], app=self.app
        )
        self.assertAllCalled()

    def test_015_disp_shortcut(self):
        self.app.expected_calls[
            ("dom0", "admin.vm.Create.DispVM", None, b"name=new-vm label=red")
        ] = b"0\x00"
        self.app.expected_calls[
            ("dom0", "admin.label.List", None, None)
        ] = b"0\x00red\nblue\n"
        ...
        qubesadmin.tools.qvm_create.main(["--disp", "new-vm"], app=self.app)
        self.assertAllCalled()
