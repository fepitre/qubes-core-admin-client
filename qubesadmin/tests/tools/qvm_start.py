...
class TC_00_qvm_start(qubesadmin.tests.QubesTestCase):
    ...
    def test_003_already_running(self):
        ...
        self.assertEqual(
            qubesadmin.tools.qvm_start.main(["some-vm"], app=self.app), 1
        )
        self.assertAllCalled()
    ...
    def test_010_drive_cdrom(self):
        ...
        qubesadmin.tools.qvm_start.main(
            ["--cdrom=dom0:sr0", "some-vm"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_011_drive_disk(self):
        ...
        self.app.expected_calls[
            (
                ...
                "admin.vm.device.block.Attach",
                "dom0+sdb1",
                b"devtype=disk persistent=True read-only=False",
            )
        ] = b"0\x00"
        self.app.expected_calls[
            (
        ...
        qubesadmin.tools.qvm_start.main(
            ["--hd=dom0:sdb1", "some-vm"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_012_drive_disk(self):
        ...
        qubesadmin.tools.qvm_start.main(
            ["--drive=hd:dom0:sdb1", "some-vm"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_013_drive_loop_local(self, mock_subprocess):
        ...
        qubesadmin.tools.qvm_start.main(
            ["--cdrom=dom0:/home/some/image.iso", "some-vm"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_014_drive_loop_remote(self):
        ...
        with unittest.mock.patch.object(
            self.app.domains["other-vm"], "run"
        ) as mock_run:
            mock_run.return_value = (b"/dev/loop7", b"")
            qubesadmin.tools.qvm_start.main(
                ["--cdrom=other-vm:/home/some image.iso", "some-vm"],
                app=self.app,
            )
            mock_run.assert_called_once_with(
                "losetup -f --show '/home/some image.iso'", user="root"
            )
        self.assertAllCalled()
    ...
    def test_015_drive_failed_start(self):
        ...
        self.app.expected_calls[
            ("some-vm", "admin.vm.device.block.Detach", "other-vm+loop7", None)
        ] = b"0\x00"
        qubesadmin.tools.qvm_start.main(
            ["--cdrom=other-vm:loop7", "some-vm"], app=self.app
        )
        self.assertAllCalled()
