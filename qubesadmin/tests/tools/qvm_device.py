...
class TC_00_qvm_device(qubesadmin.tests.QubesTestCase):
    ...
    def setUp(self):
        ...
        self.app.expected_calls[
            ("test-vm1", "admin.vm.device.test.Available", None, None)
    ...
    def test_000_list_all(self):
        ...
        self.app.expected_calls[
            ("test-vm1", "admin.vm.device.test.List", None, None)
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm2", "admin.vm.device.test.List", None, None)
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm3", "admin.vm.device.test.List", None, None)
        ] = b"0\0"

        with qubesadmin.tests.tools.StdoutBuffer() as buf:
            qubesadmin.tools.qvm_device.main(["test", "list"], app=self.app)
            self.assertEqual(
    ...
    def test_001_list_persistent_attach(self):
        ...
        with qubesadmin.tests.tools.StdoutBuffer() as buf:
            ...
            self.assertEqual(
                buf.getvalue(), "test-vm1:dev1  Description here  test-vm3\n"
            )
    ...
    def test_010_attach(self):
        """ Test attach action """
        self.app.expected_calls[
            ("test-vm2", "admin.vm.device.test.Attach", "test-vm1+dev1", b"")
        ] = b"0\0"
        qubesadmin.tools.qvm_device.main(
    ...
    def test_011_attach_persistent(self):
        ...
        qubesadmin.tools.qvm_device.main(
            ["test", "attach", "-p", "test-vm2", "test-vm1:dev1"], app=self.app
        )
        self.assertAllCalled()
    ...
    def test_012_attach_invalid(self):
        ...
        with qubesadmin.tests.tools.StderrBuffer() as stderr:
            with self.assertRaises(SystemExit):
                qubesadmin.tools.qvm_device.main(
                    ["test", "attach", "-p", "test-vm2", "dev1"], app=self.app
                )
            self.assertIn(
                "expected a backend vm & device id", stderr.getvalue()
            )
        self.assertAllCalled()
    ...
    def test_013_attach_invalid2(self):
        ...
        with qubesadmin.tests.tools.StderrBuffer() as stderr:
            ...
            self.assertIn("doesn't expose device", stderr.getvalue())
        self.assertAllCalled()
    ...
    def test_020_detach(self):
        """ Test detach action """
        self.app.expected_calls[
            ("test-vm2", "admin.vm.device.test.Detach", "test-vm1+dev1", None)
        ] = b"0\0"
        qubesadmin.tools.qvm_device.main(
    ...
    def test_021_detach_unknown(self):
        """ Test detach action """
        self.app.expected_calls[
            ("test-vm2", "admin.vm.device.test.Detach", "test-vm1+dev7", None)
        ] = b"0\0"
        qubesadmin.tools.qvm_device.main(
    ...
    def test_022_detach_all(self):
        ...
        self.app.expected_calls[
            ("test-vm2", "admin.vm.device.test.Detach", "test-vm1+dev2", None)
        ] = b"0\0"
        qubesadmin.tools.qvm_device.main(
