...
class TC_00_qvm_start_gui(qubesadmin.tests.QubesTestCase):
    ...
    def test_022_start_gui_for_vm_hvm_stubdom(self):
        ...
        patch_args = unittest.mock.patch.object(
            self.launcher, "common_guid_args", lambda vm: []
        )
        patch_pidfile = unittest.mock.patch.object(
            self.launcher, "guid_pidfile", lambda vm: pidfile.name
        )
        try:
            ...
            # common arguments dropped for simplicity
            mock_proc.assert_called_once_with("-d", "3000", "-n", "-K", "1234")
        finally:
    ...
    def test_040_start_gui(self):
        ...
        patch_start_vm = unittest.mock.patch.object(
            self.launcher,
            "start_gui_for_vm",
            functools.partial(self.mock_coroutine, mock_start_vm),
        )
        patch_start_stubdomain = unittest.mock.patch.object(
            self.launcher,
            "start_gui_for_stubdomain",
            lambda vm_, force: self.mock_coroutine(mock_start_stubdomain, vm_),
        )
        try:
...
""".splitlines()
        self.assertEqual(
            qubesadmin.tools.qvm_start_gui.get_monitor_layout(),
            ["1920 1200 0 0\n"],
        )
...
""".splitlines()
    ...
    def test_060_send_monitor_layout(self):
        ...
        patch_run_service = unittest.mock.patch.object(
            qubesadmin.vm.QubesVM, "run_service_for_stdio", mock_run_service
        )
    ...
    def test_063_send_monitor_layout_signal_existing(self):
        ...
        with tempfile.NamedTemporaryFile() as pidfile:
            ...
            self.assertEqual(
                mock_guid_pidfile.mock_calls,
                [unittest.mock.call(123), unittest.mock.call(124)],
            )
            self.assertEqual(
    ...
    def test_070_send_monitor_layout_all(self):
        ...
        self.app.expected_calls[
            ("test-vm3", "admin.vm.feature.CheckWithTemplate", "gui", None)
        ] = b"0\x00"
        self.app.expected_calls[
        ...
        self.assertCountEqual(
            mock_send_monitor_layout.mock_calls,
            [
