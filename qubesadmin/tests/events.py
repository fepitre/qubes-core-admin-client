...
class TC_00_Events(qubesadmin.tests.QubesTestCase):
    ...
    def test_010_listen_for_events(self):
        ...
        self.dispatcher._get_events_reader = lambda vm: self.mock_get_events_reader(
            stream, cleanup_func, None, vm
        )
        handler = unittest.mock.Mock()
        self.dispatcher.add_handler("some-event", handler)
        events = [
        ...
        loop.run_until_complete(
            self.dispatcher.listen_for_events(reconnect=False)
        )
        self.assertEqual(
            handler.mock_calls,
            [
                ...
                unittest.mock.call(
                    self.app.domains.get_blind("some-vm"),
                    "some-event",
                    arg1="value1",
                ),
                unittest.mock.call(
    ...
    def test_020_get_events_reader_local(self):
        ...
        with unittest.mock.patch(
            ...
            reader = asyncio.ensure_future(
                loop.run_in_executor(None, self.read_all, sock2)
            )
            loop.run_until_complete(asyncio.wait([task, reader]))
            self.assertEqual(reader.result(), b"dom0\0admin.Events\0dom0\0\0")
    ...
    def test_021_get_events_reader_local_vm(self):
        ...
        with unittest.mock.patch(
            ...
            loop.run_until_complete(asyncio.wait([task, reader]))
            self.assertEqual(
                reader.result(), b"dom0\0admin.Events\0test-vm\0\0"
            )
            self.assertIsInstance(task.result()[0], asyncio.StreamReader)
            cleanup_func = task.result()[1]
            cleanup_func()
    ...
    def test_022_get_events_reader_remote(self):
        ...
        with unittest.mock.patch(
            ...
            lambda *args, **kwargs: self.mock_coroutine(
                mock_proc, *args, **kwargs
            ),
        ):
            task = asyncio.ensure_future(self.dispatcher._get_events_reader())
    ...
    def test_030_events_device(self):
        ...
        self.dispatcher.handle(
            "test-vm", "device-attach:test", device="test-vm2:dev", options="{}"
        )
        vm = self.app.domains.get_blind("test-vm")
        dev = self.app.domains.get_blind("test-vm2").devices["test"]["dev"]
        handler.assert_called_once_with(
            vm, "device-attach:test", device=dev, options="{}"
        )
