...
class TC_00_VMCollection(qubesadmin.tests.QubesTestCase):
    def test_000_list(self):
        ...
        self.assertEqual(list(self.app.domains.keys()), ["test-vm"])
    ...
    def test_004_delitem(self):
        self.app.expected_calls[
            ("test-vm", "admin.vm.Remove", None, None)
        ] = b"0\x00"
    ...
    def test_005_keys(self):
        ...
        self.assertEqual(
            set(self.app.domains.keys()), set(["test-vm", "test-vm2"])
        )
        self.assertAllCalled()
    ...
    def test_006_values(self):
        ...
        self.assertEqual(
            set([vm.name for vm in values]), set(["test-vm", "test-vm2"])
        )
        self.assertAllCalled()
...
class TC_10_QubesBase(qubesadmin.tests.QubesTestCase):
    def setUp(self):
        ...
        self.check_output_patch = mock.patch("subprocess.check_output")
    ...
    def test_010_new_simple(self):
        self.app.expected_calls[
            ("dom0", "admin.vm.Create.AppVM", None, b"name=new-vm label=red")
        ] = b"0\x00"
        self.app.expected_calls[
            ("dom0", "admin.vm.List", None, None)
    ...
    def test_013_new_objects_params(self):
        ...
        vm = self.app.add_new_vm(
            self.app.get_vm_class("AppVM"),
            "new-vm",
            self.app.get_label("red"),
            self.app.domains["some-template"],
        )
        self.assertEqual(vm.name, "new-vm")
        self.assertEqual(vm.klass, "AppVM")
        self.assertAllCalled()
    ...
    def test_014_new_pool(self):
        self.app.expected_calls[
            (
                ...
                None,
                b"name=new-vm label=red pool=some-pool",
            )
        ] = b"0\x00"
        self.app.expected_calls[
            ("dom0", "admin.vm.List", None, None)
    ...
    def test_016_new_template_based_default(self):
        ...
        vm = self.app.add_new_vm(
            "AppVM", "new-vm", "red", template=qubesadmin.DEFAULT
        )
        self.assertEqual(vm.name, "new-vm")
        self.assertEqual(vm.klass, "AppVM")
        self.assertAllCalled()
    ...
    def clone_setup_common_calls(self, src, dst):
        ...
        self.app.expected_calls[(src, "admin.vm.property.List", None, None)] = (
            b"0\0qid\nname\n"
        ...
        for prop, value in properties.items():
            ...
            if "default=True" in value:
                continue
            self.app.expected_calls[
                (dst, "admin.vm.property.Set", prop, value.split()[-1].encode())
            ] = b"0\0"
        ...
        self.app.expected_calls[
            (dst, "admin.vm.tag.Set", "tag1", None)
        ] = b"0\0"
        self.app.expected_calls[
            (dst, "admin.vm.tag.Set", "tag2", None)
        ] = b"0\0"
        ...
        self.app.expected_calls[
            (dst, "admin.vm.feature.Set", "feat1", b"feat1-value with spaces")
        ] = b"0\0"
        self.app.expected_calls[
            (dst, "admin.vm.feature.Set", "feat2", b"1")
        ] = b"0\0"

        # firewall
        rules = b"action=drop dst4=192.168.0.0/24\n" b"action=accept\n"
        self.app.expected_calls[(src, "admin.vm.firewall.Get", None, None)] = (
            b"0\x00" + rules
        )
        self.app.expected_calls[
    ...
    def test_032_clone_pool(self):
        ...
        self.app.expected_calls[
            (
                ...
                "admin.vm.CreateInPool.AppVM",
                "test-template",
                b"name=new-name label=red pool=some-pool",
            )
        ] = b"0\x00"
        self.app.expected_calls[("dom0", "admin.vm.List", None, None)] = (
            b"0\x00new-name class=AppVM state=Halted\n"
    ...
    def test_034_clone_class_change(self):
        ...
        new_vm = self.app.clone_vm(
            "test-vm", "new-name", new_cls="StandaloneVM"
        )
        self.assertEqual(new_vm.name, "new-name")
    ...
    def test_035_clone_fail(self):
        ...
        self.app.expected_calls[
            ("new-name", "admin.vm.Remove", None, None)
        ] = b"0\x00"
    ...
    def test_040_clone_ignore_errors_storage(self):
        ...
        self.app.expected_calls[
            ("new-name", "admin.vm.volume.CloneTo", "private", b"token-private")
        ] = b"2\0QubesException\0\0something happened\0"
        self.app.expected_calls[
            ("new-name", "admin.vm.Remove", None, None)
        ] = b"0\x00"
    ...
    def test_042_clone_nondefault_pool(self):
        ...
        self.app.expected_calls[
            (
                ...
                "admin.vm.CreateInPool.AppVM",
                "test-template",
                b"name=new-name label=red pool:private=another",
            )
        ] = b"0\x00"
        new_vm = self.app.clone_vm("test-vm", "new-name")
        self.assertEqual(new_vm.name, "new-name")
...
class TC_20_QubesLocal(unittest.TestCase):
    ...
    def listen_and_send(self, send_data):
        ...
        self.proc = multiprocessing.Process(
            target=worker, args=(self.socket, child_pipe, send_data)
        )
        self.proc.start()
        self.socket.close()
    ...
    def test_000_qubesd_call(self):
        ...
        self.assertEqual(
            self.get_request(), b"dom0\0some.method\0test-vm\0arg1\0payload"
        )
    ...
    def test_001_qubesd_call_none_arg(self):
        ...
        self.assertEqual(
            self.get_request(), b"dom0\0some.method\0test-vm\0\0payload"
        )

    def test_002_qubesd_call_none_payload(self):
        self.listen_and_send(b"0\0")
        self.app.qubesd_call("test-vm", "some.method", None, None)
        self.assertEqual(self.get_request(), b"dom0\0some.method\0test-vm\0\0")
    ...
    def test_003_qubesd_call_payload_stream(self):
        ...
        with open(payload_input, "w+") as payload_file:
            ...
            payload_file.seek(0)
            with mock.patch("qubesadmin.config.QREXEC_SERVICES_DIR", tmpdir):
    ...
    def test_004_qubesd_call_payload_stream_proc(self):
        ...
        echo = subprocess.Popen(
            ["echo", "some payload"], stdout=subprocess.PIPE
        )
        with open(service_path, "w") as f:
            f.write(
                "#!/bin/bash\n"
    ...
    def test_010_run_service(self):
        ...
        self.assertEqual(
            self.get_request(), b"dom0\0admin.vm.Start\0some-vm\0\0"
        )
...
class TC_30_QubesRemote(unittest.TestCase):
    def setUp(self):
        ...
        self.proc_mock.configure_mock(**{"return_value.returncode": 0})
    ...
    def test_013_run_service_default_target(self):
        self.app.run_service("", "service.name")
        self.proc_mock.assert_called_once_with(
            [qubesadmin.config.QREXEC_CLIENT_VM, "-T", "", "service.name"],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
    ...
    def test_015_run_service_no_autostart2(self):
        ...
        with self.assertRaises(qubesadmin.exc.QubesVMNotRunningError):
            self.app.run_service("some-vm", "service.name", autostart=False)
        self.proc_mock.assert_called_once_with(
            [qubesadmin.config.QREXEC_CLIENT_VM, "some-vm", "admin.vm.List"],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
