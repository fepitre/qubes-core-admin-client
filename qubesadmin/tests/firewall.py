...
class TC_06_Expire(qubesadmin.tests.QubesTestCase):
    def test_000_number(self):
        ...
        self.assertEqual(
            instance.datetime, datetime.datetime(2016, 5, 15, 6, 7, 32)
        )
...
class TC_10_Rule(qubesadmin.tests.QubesTestCase):
    ...
    def test_001_expire(self):
        ...
        self.assertEqual(
            rule.rule, "action=accept proto=icmp expire=1463292452"
        )
    ...
    def test_003_reject_invalid(self):
        ...
        with self.assertRaises(ValueError):
            ...
            qubesadmin.firewall.Rule(
                None, action="accept", proto="tcp", icmptype=8
            )
        with self.assertRaises(ValueError):
            # not proto=icmp for icmptype
    ...
    def test_005_parse_str(self):
        rule_txt = "action=accept dst4=192.168.0.0/24 proto=tcp dstports=443"
        with self.assertNotRaises(ValueError):
    ...
    def test_006_parse_str_comment(self):
        rule_txt = "action=accept dsthost=qubes-os.org comment=Some comment"
        with self.assertNotRaises(ValueError):
            rule = qubesadmin.firewall.Rule(rule_txt)
    ...
    def test_020_set_rules(self):
        ...
        self.app.expected_calls[
            (
                ...
                None,
                "".join(rule + "\n" for rule in rules_txt).encode("ascii"),
            )
        ] = b"0\0"
