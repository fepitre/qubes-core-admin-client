        ...
        if volume.domains:
            ...
            for tupple in volume.domains:
                ...
                if full or not sys.stdout.isatty():
                    output += [
                        (str(volume), vmname, volume_name, volume.revisions)
                    ]
                else:
...
def list_volumes(args):
    ...
    if hasattr(args, "domains") and args.domains:
        result = [
            x  # reduce to only VolumeData with assigned domains
            for p in vd_dict.values()
            for x in p.values()
            if x.domains
        ]
    else:
...
def revert_volume(args):
    ...
    if args.revision:
        ...
        revisions = volume.revisions
        if not revisions:
            raise qubesadmin.exc.StoragePoolException("No snapshots available")
...
def extend_volumes(args):
    ...
    if not args.force and size < volume.size:
        raise qubesadmin.exc.StoragePoolException(
            ...
            "option." % (volume.name, size, volume.size)
        )
    volume.resize(size)
...
def init_list_parser(sub_parsers):
    ...
    list_parser = sub_parsers.add_parser(
        "list", aliases=("ls", "l"), help="list storage volumes"
    )
    list_parser.add_argument(
        "-p", "--pool", dest="pools", action=qubesadmin.tools.PoolsAction
    )
    list_parser.add_argument(
        "--full",
        action="store_true",
...
def init_revert_parser(sub_parsers):
    ...
    revert_parser = sub_parsers.add_parser(
        "revert", aliases=("rv", "r"), help="revert volume to previous revision"
    )
...
def init_import_parser(sub_parsers):
    ...
    import_parser = sub_parsers.add_parser("import", help="import volume data")
...
def get_parser():
    ...
    parser = qubesadmin.tools.QubesArgumentParser(
        description=__doc__, want_app=True
    )
    parser.register(
