...
no_profile = group.add_argument_group(
    "Profile setup", "Manually specify profile options"
)
no_profile.add_argument(
    "--exclude",
    "-x",
    action="append",
    dest="exclude_list",
    default=[],
    help="Exclude the specified VM from the backup (may be " "repeated)",
)
no_profile.add_argument(
    "--dest-vm",
    "-d",
...
no_profile.add_argument(
    ...
    default=None,
    help="Read passphrase from a file, or use '-' to read " "from stdin",
)
no_profile.add_argument(
    "--compress",
    "-z",
...
no_profile.add_argument(
    ...
    action="store",
    dest="compression",
    help="Specify a non-default compression filter program " "(default: gzip)",
)
no_profile.add_argument(
    "--save-profile",
    action="store",
...
def print_progress(
    expected_profile, _subject, _event, backup_profile, progress
):
...
def main(args=None, app=None):
    ...
    if args.profile is None:
        ...
        if args.app.qubesd_connection_type == "socket":
            ...
            if args.save_profile:
                ...
                return 1
            print(
                "To perform the backup according to selected options, "
    ...
    if args.profile is None:
        if args.passphrase_file is not None:
            ...
            prompt = (
                "Please enter the passphrase that will be used to "
    ...
    if have_events:
        ...
        events_dispatcher.add_handler(
            "backup-progress", functools.partial(print_progress, profile_name)
        )
