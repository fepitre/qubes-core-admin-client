...
parser.add_argument(
    "--disp", action="store_true", help="alias for --class DispVM --label red"
)
...
# silently ignored
parser_root.add_argument(
    "--no-root", action="store_true", default=False, help=argparse.SUPPRESS
)
...
def main(args=None, app=None):
    ...
    if root_source_path and args.cls in ("AppVM", "DispVM"):
        parser.error(
            "--root-copy-from/--root-move-from used but this qube "
    ...
    try:
        ...
        parser.error(
            "no such label: {!r}; available: {}".format(
                args.properties["label"], ", ".join(args.app.labels)
            )
        )
    ...
    try:
        if args.cls == "StandaloneVM" and "template" in args.properties:
            ...
            # the template
            vm = args.app.clone_vm(
                args.properties.pop("template"),
    ...
    for prop, value in args.properties.items():
        try:
            ...
            args.app.log.error(
                "Error setting property {} (but VM created): {!s}".format(
                    prop, e
                )
            )
            retcode = 2
    ...
    if root_source_path:
        try:
            ...
            args.app.log.error(
                "Error importing root volume (but VM created): {}".format(e)
            )
            retcode = 3
