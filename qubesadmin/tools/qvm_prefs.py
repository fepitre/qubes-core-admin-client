...
def get_parser(vmname_nargs=1):
    ...
    parser = qubesadmin.tools.QubesArgumentParser(vmname_nargs=vmname_nargs)
    ...
    parser_value.add_argument(
        "value", metavar="VALUE", nargs="?", help="new value of the property"
    )
...
def process_actions(parser, args, target):
    ...
    if args.help_properties:
        ...
        for prop in sorted(properties):
            ...
            print(
                wrapper.fill(
                    "{name:{width}s}  {help_text!s}".format(
    ...
    if args.property is None:
        ...
        for prop in sorted(properties):
            try:
                ...
                print("{name:{width}s}  U".format(name=prop, width=width))
                continue

            if not target.property_is_default(prop):
                print(
                    "{name:{width}s}  -  {value!s}".format(
                        name=prop, width=width, value=value
                    )
                )
            elif not args.hide_default:
                print(
                    "{name:{width}s}  D  {value!s}".format(
                        name=prop, width=width, value=value
                    )
                )
